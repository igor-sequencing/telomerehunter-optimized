...
def sort_telomere_reads(input_dir, band_file, out_dir, pid, mapq_threshold, repeats):
	...
	##################################################
	### sort and index fraction BAMs for pysam    ###
	##################################################

	print "Sorting and indexing fraction BAMs..."

	# Define fraction BAM paths
	fraction_bams = [
		out_dir + "/" + pid + "_filtered_intratelomeric.bam",
		out_dir + "/" + pid + "_filtered_junctionspanning.bam",
		out_dir + "/" + pid + "_filtered_subtelomeric.bam",
		out_dir + "/" + pid + "_filtered_intrachromosomal.bam"
	]

	# Sort and index each fraction BAM
	for bam_path in fraction_bams:
		if os.path.exists(bam_path):
			# Sort by coordinate
			sorted_path = bam_path.replace(".bam", "_sorted_tmp.bam")
			sort_cmd = "samtools sort \"" + bam_path + "\" -o \"" + sorted_path + "\""

			try:
				subprocess.check_call(sort_cmd, shell=True, stderr=subprocess.PIPE)
				...
				print "Warning: Failed to sort/index " + bam_path
			except Exception as e:
				print "Warning: Error processing " + bam_path + ": " + str(e)

	print "Fraction BAMs sorted and indexed"
